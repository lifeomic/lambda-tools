...
test.beforeEach((t) => {
  ...
  Object.assign(t.context, { debugSpy, docker, dockerPull });
});
...
test.serial('will debug the progress of pulling an image', async (t) => {
  const { docker, debugSpy, dockerPull } = t.context;

  await pullImage(docker, TEST_IMAGE);
  sinon.assert.called(debugSpy);
  sinon.assert.calledWithExactly(debugSpy, `${TEST_IMAGE}: Status: Downloaded newer image for ${TEST_IMAGE}`);
